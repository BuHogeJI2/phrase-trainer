...
export function ProgressSummary({ known, studying, difficult, total, quizTotal, quizAccuracy, streak }: ProgressSummaryProps) {
  const progressPercent = total === 0 ? 0 : Math.round((known / total) * 100)
    ...
    <section className="rounded-[28px] border border-[var(--color-border)] bg-[var(--color-surface)] p-5 shadow-[var(--shadow-soft)]">
      <div className="flex items-center justify-between gap-3">
        <div>
          ...
          <p className="mt-1 text-sm text-[var(--color-text-muted)]">
            Уверенно знаете: {known} из {total} ({progressPercent}%)
          </p>
        </div>
      ...
      <div className="mt-4 grid grid-cols-3 gap-2 text-center text-sm">
        <div className="rounded-2xl bg-[var(--color-surface-muted)] p-3">
          <p className="text-xs text-[var(--color-text-muted)]">В работе</p>
          <p className="mt-1 text-base font-semibold text-[var(--color-text)]">{studying}</p>
        </div>
        <div className="rounded-2xl bg-[var(--color-surface-muted)] p-3">
          <p className="text-xs text-[var(--color-text-muted)]">Трудные</p>
          <p className="mt-1 text-base font-semibold text-[var(--color-text)]">{difficult}</p>
        </div>
        <div className="rounded-2xl bg-[var(--color-surface-muted)] p-3">
