...
- `PhraseCard.tsx` is the main interaction component for reveal, difficult/known marking, saving, and audio playback.
...
- `progress`: per-phrase learning state, saved phrases, aggregate quiz stats, and last visited situation metadata.
...
- Daily practice is deterministic for a given ISO date, but the pool is now prioritized by difficult, unstable, new, and known phrases instead of pure random order.
...
- Practice is structured as `learn -> checkpoint -> mistakes -> retry -> done`.
- The checkpoint combines one matching round plus multiple-choice recall for the current 5-phrase block.
- Wrong answers feed an immediate correction loop and also influence later repetition priority.
- Revealing a phrase on a card records exposure only; verified learning comes from active recall.
...
- The manifest currently reuses the branded SVG favicon; dedicated raster install icons are still a follow-up item.
